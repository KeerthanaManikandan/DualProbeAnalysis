%% combinedAnalysisEphysDualProbeRS_v2
...
for iM = 1:2
        allMonkeyVarsTemp =  load(['D:\Data\' monkeys{iM} '_SqM\Left Hemisphere\DualProbeVars.mat']);

    if ~isfield(allMonkeyVarsTemp,'corticalAreaProbeA') || isempty(allMonkeyVarsTemp.corticalAreaProbeA) ...
           || isempty(allMonkeyVarsTemp.corticalAreaProbeB) || ~isfield(allMonkeyVarsTemp,'termination3b') || ...
           ~isfield(allMonkeyVarsTemp,'terminationArea4') 
 
        clear allDates datFileNumAll corticalAreaProbeA corticalAreaProbeB termination3b terminationArea4
        [allDates,datFileNumAll,~,~,~,~,~,~, ~ , ~,~,~,~,~,~,~,~,corticalAreaProbeA,corticalAreaProbeB] = ...
            getMonkeyParamsDualProbeEphys(monkeys{iM},commonDir);     
        ...
        % 
        % allMonkeyVars(iM).corticalAreaProbeA = corticalAreaProbeA;
        % allMonkeyVars(iM).corticalAreaProbeB  = corticalAreaProbeB;
        % allMonkeyVars(iM).termination3b       = termination3b;
        % allMonkeyVars(iM).terminationArea4    = terminationArea4;

        termination3b = reshape(termination3b',[numel(termination3b) 1]);
        termination3b(allMonkeyVarsTemp.removeDataIdx) = []; %termination3b(singleChRow) = [];
        terminationArea4 = reshape(terminationArea4',[numel(terminationArea4) 1]);
        terminationArea4(allMonkeyVarsTemp.removeDataIdx) = []; %terminationArea4(singleChRow) = [];       

        save(['D:\Data\' monkeys{iM} '_SqM\Left Hemisphere\DualProbeVars.mat'],'corticalAreaProbeA',...
            'corticalAreaProbeB','termination3b','terminationArea4','-append');
    end

    allMonkeyVars(iM) =  load(['D:\Data\' monkeys{iM} '_SqM\Left Hemisphere\DualProbeVars.mat']);
    fieldNames = fieldnames(allMonkeyVars(iM));
    ...
    for iL = 1:length(fieldNames)
        if (contains(fieldNames{iL},'Super') || contains(fieldNames{iL},'Mid') || contains(fieldNames{iL},'Deep'))
            allMonkeyVars(iM).(fieldNames{iL})(singleChRow,:) = [];
        end
    end

end
clear allMonkeyVarsTemp
...
%%
figure;
medVal = squeeze(median(meanSpecAll,1,'omitnan'));  % 5x21
sem    = squeeze(mad(meanSpecAll,1))./sqrt(size(meanSpecAll,1));  % 5x21

for iBand = 1:5
    subplot(2,3,iBand);
    medVal_freq = medVal(iBand, :);  % 1x21
    sem_freq = sem(iBand, :);        % 1x21

    plot(medVal_freq, 1:21); hold on;
    patch([medVal_freq-2*sem_freq, fliplr(medVal_freq+2*sem_freq)], ...
        [(1:21) fliplr(1:21)],'blue','FaceAlpha', 0.3, 'EdgeColor', 'none');
    title(bandLabels{iBand});xlim([0.2 1]);
    box off; set(gca,'YDir','reverse');
    xlabel('Power (au)'); ylabel('Channels');
end
...
cohAll        = cat(1,allCohVals(:).coh);
confAll       = cat(1,allCohVals(:).confThresh);
confJackknife = cat(1,allCohVals(:).confIntJackknife);
f             = allCohVals(1).freqVals;
combAllCh     = cat(1,allCohVals(:).combAllCh);
...
% typeLabel = 'Power';
% figure; subplot(121);
% boxplot([superVal(flagVal,4) midVal(flagVal,4) deepVal(flagVal,4) ...
%     superMid(flagVal,4) superDeep(flagVal,4) midDeep(flagVal,4)],{'S/S','M/M','D/D',...
%     'S/M','S/D','M/D'}); box off;  ylim([-0.05 0.7]);
% 
% 
% subplot(122);
% boxplot([[superVal(flagVal,4); superMid(flagVal,4); superDeep(flagVal,4)] ...
%     [midVal(flagVal,4); superMid(flagVal,4); midDeep(flagVal,4)] ...
%     [deepVal(flagVal,4); midDeep(flagVal,4); superDeep(flagVal,4)]],{'Superficial','Middle','Deep'});
% box off; ylim([-0.05 0.7]);
% 
% [~,pAll,stAll] = anova1([superVal(flagVal,4) midVal(flagVal,4) deepVal(flagVal,4) ...
%     superMid(flagVal,4) superDeep(flagVal,4) midDeep(flagVal,4)],{'S/S','M/M','D/D',...
%     'S/M','S/D','M/D'});
% 
% [~,pS,stS] = anova1([[superVal(flagVal,4); superMid(flagVal,4); superDeep(flagVal,4)] ...
%     [midVal(flagVal,4); superMid(flagVal,4); midDeep(flagVal,4)] ...
%     [deepVal(flagVal,4); midDeep(flagVal,4); superDeep(flagVal,4)]],{'Superficial','Middle','Deep'});
% %
% % figure; 
% % subplot(231); imagesc(1:5,1:3,superAllCorr); title('S/S');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]); colorbar;
% % subplot(232); imagesc(1:5,1:3,superMidCorr); title('S/M');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]);colorbar;
% % subplot(233); imagesc(1:5,1:3,superDeepCorr); title('S/D');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]);colorbar;
% % subplot(234); imagesc(1:5,1:3,midAllCorr); title('M/M');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]);colorbar;
% % subplot(235); imagesc(1:5,1:3,midDeepCorr); title('M/D');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]);colorbar;
% % subplot(236); imagesc(1:5,1:3,deepAllCorr); title('D/D');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.2 0.35]);colorbar;
% % 
% % %
% % figure;
% % subplot(131); imagesc(1:5,1:3,superRefCorr); title('Superficial');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.1 0.35]); colorbar;
% % subplot(132); imagesc(1:5,1:3,midRefCorr); title('Middle');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.1 0.35]); colorbar;
% % subplot(133); imagesc(1:5,1:3,deepRefCorr); title('Deep');axis square; xticklabels(bandLabels); yticklabels(timeLabels); clim([-0.1 0.35]); colorbar;
% 
% 
% figure; imagesc([corr([[superVal(flagVal,4); superMid(flagVal,4); superDeep(flagVal,4)] ...
%     [midVal(flagVal,4); superMid(flagVal,4); midDeep(flagVal,4)] ...
%     [deepVal(flagVal,4); midDeep(flagVal,4); superDeep(flagVal,4)]], repmat(connValsR(flagVal),[3 1]),'Type','Spearman') ]);xticks(1);yticks(1:3);
% yticklabels({'Superficial','Middle','Deep'}); colorbar; clim([0 0.35]);
% xticklabels({'Amplitude low G','Phase Low G','Amplitude High G','Phase High G'});

end


%% 3x3 matrix for all frequencies
...
for iM = 1:3
    if iM<3
        ...
        lowGammaThetaBtw  = pacVals(iM).thetaLGBtwT;
        highGammaThetaBtw = pacVals(iM).thetaHGBtwT;

        lowGammaThetaWth  = pacVals(iM).thetaLGWthT;
        highGammaThetaWth = pacVals(iM).thetaHGWthT;

        avgMIBtw = pacVals(iM).avgMIBetweenT;
        avgMIWth = pacVals(iM).avgMIWithinT;
        ...
        lowGammaThetaBtw  = cell2mat({pacVals.thetaLGBtwT}');
        highGammaThetaBtw = cell2mat({pacVals.thetaHGBtwT}');

        lowGammaThetaWth  = cell2mat({pacVals.thetaLGWthT}');
        highGammaThetaWth = cell2mat({pacVals.thetaHGWthT}');

        ctrlMIBtw  = cell2mat({pacVals.ctrlMIBtwT}');
        ctrlMIWth  = cell2mat({pacVals.ctrlMIWithinT}');
